...
* **任务列表:**
  - [x] 初始化 `Vue + Vite + TS + Pinia + Vuetify` 的项目环境；
  - [x] 构建项目目录结构；
  - [x] 根据核心数据结构，构建 10 个作品及其各自对应的人物数据；
  - [x] 配置 `Vite Proxy`；
  - [x] 配置网站路由；
...
* **任务列表:**
  1.  **完善API接口层:** 在 `src/api` 中增加 `getWorkById`、`getCharactersByIds` 等新函数，用于根据ID获取单个作品及其关联角色的详细信息。
